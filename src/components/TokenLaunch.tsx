...
export default function TokenLaunch({ onClose, onSuccess, username }: TokenLaunchProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-[#141414] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-white/10">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
          <h2 className="text-lg font-bold text-white">Launch Creator Token</h2>
          <p className="text-xs text-zinc-400">Powered by Bags.fm</p>
            </div>
          </div>
        ...
        {step === "success" && result && (
          <div className="p-6 text-center">
            ...
            <h3 className="text-xl font-bold text-white mb-2">Token Launched! 🎉</h3>
            <p className="text-sm text-zinc-400 mb-4">Your creator token is now live on Solana</p>

            <div className="bg-zinc-900 rounded-xl p-4 mb-4 text-left border border-white/10">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-zinc-400">Token Mint</span>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={onClose}
                className="flex-1 py-2.5 px-4 border border-white/10 rounded-xl font-medium text-sm hover:bg-zinc-900 transition text-zinc-400"
              >
                Close
              </button>
            </div>
        ...
        {step === "form" && (
          <div className="p-5 space-y-4">
            ...
            {/* Token Name */}
            <div>
              <label className="text-xs font-medium text-zinc-400 mb-1.5 block">Token Name *</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="e.g. Shaan Token"
                className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
              />
            </div>

            {/* Symbol */}
            <div>
              <label className="text-xs font-medium text-zinc-400 mb-1.5 block">Symbol *</label>
              <div className="relative">
                ...
                <input
                  ...
                  maxLength={6}
                  className="w-full pl-7 pr-4 bg-zinc-900 border border-white/10 rounded-xl py-2.5 text-sm font-mono text-white focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white resize-none focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
              />
            </div>

            {/* Token Image */}
            <div>
              <label className="text-xs font-medium text-zinc-400 mb-1.5 block">Token Image *</label>
              {imageUrl ? (
                <div className="flex items-center gap-3 bg-zinc-900 border border-white/10 rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-zinc-400 truncate">{imageFile?.name || "Image uploaded"}</p>
                    <button
                      type="button"
                ...
                <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-white/10 rounded-xl cursor-pointer hover:border-[#2563EB]/40 hover:bg-[#EFF6FF]/50 transition">
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-medium text-zinc-400 mb-1 block">Twitter (optional)</label>
                <input
                  type="url"
                  value={twitter}
                  onChange={(e) => setTwitter(e.target.value)}
                  placeholder="https://x.com/..."
                  className="w-full bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
                />
              </div>
              <div>
                <label className="text-[10px] font-medium text-zinc-400 mb-1 block">Website (optional)</label>
                <input
                  type="url"
                  value={website}
            ...
            {/* Initial Buy */}
            <div>
              <label className="text-xs font-medium text-zinc-400 mb-1.5 block">Initial Buy (SOL) — optional</label>
              ...
              <div className="flex items-center gap-2 bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5">
                ...
                />
                <span className="text-xs font-medium text-zinc-400">SOL</span>
              </div>
            </div>
